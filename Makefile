...
dev-db:
	mysql -uroot -p < cookiecutter_plugin/scripts/init-db.sql
...
django-migrate:
	./manage.py migrate
	./manage.py makemigrations cookiecutter_plugin
	./manage.py migrate cookiecutter_plugin
...
build:
	...
	if [ -d "./cookiecutter_plugin.egg-info" ]; then sudo rm -r cookiecutter_plugin.egg-info; fi
...
# -------------------------------------------------------------------------
# upload to PyPi
# https://pypi.org/project/cookiecutter-openedx-plugin/
# -------------------------------------------------------------------------
release-prod:
	make build
